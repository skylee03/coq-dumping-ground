...
Definition COMB_S :=
  abs (typ_arrow typ_base (typ_arrow typ_base typ_base)) (abs (typ_arrow typ_base typ_base) (abs typ_base (app (app 2 0) (app 1 0)))).
...
Proof.
  ...
  - f_equal.
    inversion H.
    apply IHe with (j := S j) (u := u) (i := S i) (v := v); auto.
  - inversion H.
    f_equal.
    + eapply IHe1; eauto.
    + eapply IHe2; eauto.
...
Proof.
  ...
  - apply typing_app_c with (T1 := T1); auto.
Qed.
...
Proof.
  ...
  - rewrite e in *.
    ...
    assert (T = S).
    + apply binds_mid_eq with (x := z) (E := E) (F := F); auto.
    + rewrite H0; auto.
    + apply uniq_remove_mid with (F := z ~ S); auto.
  - apply typing_var_c.
    + apply uniq_remove_mid with (F := z ~ S); auto.
    + apply binds_remove_mid with (y := z) (b := S); auto.
Qed.
...
Proof.
  ...
  - pick fresh y and apply typing_abs_c.
    ...
    apply H1; auto.
    apply typing_c_to_lc_c with (E := E) (T := S).
    exact H0.
  - apply typing_app_c with (T1 := T1); auto.
Qed.
...
Proof.
  intros.
  rewrite_env (nil ++ E).
  apply typing_c_subst with (S := S); auto.
Qed.
...
Proof.
  ...
  induction H; intros e' J; inversion J; subst; eauto.
  inversion H; subst.
  pick fresh x.
  rewrite (subst_intro x); auto.
  apply typing_c_subst_simple with (S := T1); auto.
Qed.
...
Proof.
  ...
  - left.
    apply value_abs_c.
    apply typing_c_to_lc_c with (E := nil) (T := typ_arrow T1 T2).
    exact H0.
  - right.
    destruct IHtyping_c1; auto.
    + destruct IHtyping_c2; auto.
      * inversion H2; subst.
        eauto.
      * destruct H3.
        eauto.
    + destruct H2.
      exists (app x e2).
      apply eval_app_1_c; auto.
      apply typing_c_to_lc_c with (E := nil) (T := T1).
      exact H1.
Qed.
...
Proof.
  intros.
  induction H; split; try apply lc_app; eauto using value_to_lc, open_abs; apply IHeval.
Qed.
...
Proof.
  ...
  - apply binds_unique with (x := x) (E := E); auto.
  - f_equal.
    pick fresh x.
